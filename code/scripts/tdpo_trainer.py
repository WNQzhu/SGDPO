...
from tdpo_config import TDPOConfig, FDivergenceConstants, FDivergenceType
...
class TDPOTrainer(Trainer):
    ...
    def tdpo_loss(
        ...
        logits = chosen_rejected_logps_margin - self.args.tdpo_alpha * (rejected_position_kl - chosen_position_kl.detach())  # tdpo2
    ...
    def tdpo_concatenated_forward(self, model: nn.Module,
                                  ref_model: nn.Module,
                                  batch: dict[str, Union[list, torch.LongTensor]]):
        """Run the given model on the given batch of inputs, concatenating the chosen and rejected inputs together.
        ...
        if self.is_encoder_decoder:
            ...
            outputs = model(input_ids=input_ids, attention_mask=attention_mask, **model_kwargs)
            with torch.no_grad():
                ref_outputs = ref_model(input_ids=input_ids, attention_mask=attention_mask, **model_kwargs)
        ...
        #+begin_src copy sampo
        vocab_logps = logits.log_softmax(-1)
        ref_vocab_ps = ref_logits.softmax(-1)
        ref_vocab_logps = ref_vocab_ps.log()

        per_position_kl = (ref_vocab_ps * (ref_vocab_logps - vocab_logps)).sum(-1)
        per_token_logps = torch.gather(vocab_logps, dim=2, index=labels.unsqueeze(2)).squeeze(2)
        per_ref_token_logps = torch.gather(ref_vocab_logps, dim=2, index=labels.unsqueeze(2)).squeeze(2)

        logps_margin = per_token_logps - per_ref_token_logps
        
        return (logps_margin * loss_mask).sum(-1), (per_position_kl * loss_mask).sum(-1), (per_token_logps * loss_mask).sum(-1)
    ...
    def get_batch_loss_metrics(
        ...
        all_logps_margin, all_position_kl, all_logps = self.tdpo_concatenated_forward(model, self.ref_model, batch)

        chosen_logps_margin = all_logps_margin[:num_examples]
        rejected_logps_margin = all_logps_margin[num_examples:]
        chosen_position_kl = all_position_kl[:num_examples]
        rejected_position_kl = all_position_kl[num_examples:]

        chosen_logps = all_logps[:num_examples].detach()
        rejected_logps = all_logps[num_examples:].detach()

        losses, chosen_rewards, rejected_rewards = self.tdpo_loss(chosen_logps_margin, rejected_logps_margin, chosen_position_kl, rejected_position_kl)
