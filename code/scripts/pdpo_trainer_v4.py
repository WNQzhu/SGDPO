...
from pdpo_config import PDPOConfig, FDivergenceConstants, FDivergenceType
...
class PDPOTrainer(Trainer):
    ...
    #len_tensor = torch.minimum(chosen_len, rejected_len)

    #index_tensor = self.get_index_tensor(len_tensor, device)

    def get_pilot_logps(self,
                        per_token_chosen_logps,
                        per_token_ref_chosen_logps,
                        chosen_len, min_len_tensor, chosen_start, do_further_sample=True, is_chosen=True):
        # ref_logratios = torch.tensor([0], dtype=logratios.dtype, device=logratios.device)
        bs = per_token_chosen_logps.shape[0]
        pilot_logps, ref_logps = [], []
        for i in range(bs):
            min_len, real_len = min_len_tensor[i].item(), chosen_len[i].item()
            if do_further_sample:
                if is_chosen:
                    min_len = max(int(min_len * self.args.sample_ratio_r1), 1)
                else:
                    min_len = max(int(min_len * self.args.sample_ratio_r2), 1)                    
            index_1 = torch.randperm(real_len, device=min_len_tensor.device)[:min_len] + chosen_start[i]
            # index_2 = torch.randperm(real_len, device=min_len_tensor.device)[:min_len] + chosen_start[i]            
            this_pilot_logps = per_token_chosen_logps[i][index_1].sum()
            this_ref_logps = per_token_ref_chosen_logps[i][index_1].sum()
            pilot_logps.append(this_pilot_logps)
            ref_logps.append(this_ref_logps)

        return torch.tensor(pilot_logps, device=min_len_tensor.device), \
            torch.tensor(ref_logps, device=min_len_tensor.device) 
        
    def pdpo_loss(
        self,
        per_token_chosen_logps: torch.FloatTensor,
        per_token_rejected_logps: torch.FloatTensor,
        per_token_ref_chosen_logps: torch.FloatTensor,
        per_token_ref_rejected_logps: torch.FloatTensor,
            chosen_len: torch.LongTensor,
            rejected_len: torch.LongTensor,
            chosen_start: torch.LongTensor,
            rejected_start: torch.LongTensor
    ) -> tuple[torch.FloatTensor, torch.FloatTensor, torch.FloatTensor]:
        """
        Compute the DPO loss for a batch of policy and reference model log probabilities.

        Args:
            chosen_logps (`torch.FloatTensor`): per_token
                Log probabilities of the model for the chosen responses. Shape: `(batch_size, seq)`.
            rejected_logps (`torch.FloatTensor`): per_token
                Log probabilities of the model for the rejected responses. Shape: `(batch_size, seq)`.
            ref_chosen_logps (`torch.FloatTensor`):
                Log probabilities of the reference model for the chosen responses. Shape: `(batch_size,)`.
            ref_rejected_logps (`torch.FloatTensor`):
                Log probabilities of the reference model for the rejected responses. Shape: `(batch_size,)`.
        ...
        min_len_tensor = torch.minimum(chosen_len, rejected_len)
        pilot_pilot_rejected_logps,  pilot_ref_rejected_logps = self.get_pilot_logps(
            per_token_rejected_logps,
            per_token_ref_rejected_logps, rejected_len, min_len_tensor,
            rejected_start, is_chosen=False
        )

        pilot_pilot_chosen_logps,  pilot_ref_chosen_logps = self.get_pilot_logps(
            per_token_chosen_logps,
            per_token_ref_chosen_logps, chosen_len, min_len_tensor,
            chosen_start
        )

        chosen_logps = per_token_chosen_logps.sum(-1)
        rejected_logps  = per_token_rejected_logps.sum(-1)

        ref_chosen_logps = per_token_ref_chosen_logps.sum(-1)
        ref_rejected_logps = per_token_ref_rejected_logps.sum(-1)

        logratios_1 = chosen_logps - pilot_pilot_rejected_logps
        logratios_2 = pilot_pilot_chosen_logps - rejected_logps

        ref_logratios_1 = ref_chosen_logps - pilot_ref_rejected_logps
        ref_logratios_2 = pilot_ref_chosen_logps - ref_rejected_logps
        
        logratios_1 = logratios_1.to(self.accelerator.device)
        logratios_2 = logratios_2.to(self.accelerator.device)

        ref_logratios_1 = ref_logratios_1.to(self.accelerator.device)
        ref_logratios_2 = ref_logratios_2.to(self.accelerator.device)

        logits_1 = logratios_1 - ref_logratios_1
        logits_2 = logratios_2 - ref_logratios_2        
        ...
        if self.loss_type == "sigmoid":
            ...
            #)
            losses = 0.5 * (
                -F.logsigmoid(self.beta * logits_1) * (1 - self.label_smoothing)
                - F.logsigmoid(-self.beta * logits_1) * self.label_smoothing
            ) + 0.5 * (
                -F.logsigmoid(self.beta * logits_2) * (1 - self.label_smoothing)
                - F.logsigmoid(-self.beta * logits_2) * self.label_smoothing
            )
    ...
    def concatenated_forward(self, model: nn.Module, batch: dict[str, Union[list, torch.LongTensor]],
                             return_per_token_logits: bool = True):
        ...
        idx = torch.arange(loss_mask.shape[1], 0, -1, device=loss_mask.device)
        tmp = loss_mask * idx
        start_idx = torch.argmax(tmp, 1)
        len_tensor = loss_mask.sum(-1)
        output["chosen_len"] = len_tensor[:num_examples]
        output["rejected_len"] = len_tensor[num_examples:]
        output["chosen_start"] = start_idx[:num_examples]
        output["rejected_start"] = start_idx[num_examples:]
        #+end_src
    ...
    def get_batch_loss_metrics(
        ...
        if "ref_chosen_logps" in batch and "ref_rejected_logps" in batch:
            per_token_ref_chosen_logps = batch["ref_chosen_logps"]
            per_token_ref_rejected_logps = batch["ref_rejected_logps"]
        else:
            per_token_ref_chosen_logps, per_token_ref_rejected_logps = self.compute_ref_log_probs(batch)

        #losses, chosen_rewards, rejected_rewards = self.dpo_loss(
        #    model_output["chosen_logps"], model_output["rejected_logps"], ref_chosen_logps, ref_rejected_logps
        #)
        losses, chosen_rewards, rejected_rewards = self.pdpo_loss(
            model_output["chosen_logps"], model_output["rejected_logps"],
            per_token_ref_chosen_logps, per_token_ref_rejected_logps,
            model_output["chosen_len"],   model_output["rejected_len"],
            model_output["chosen_start"], model_output["rejected_start"]
        )
