...
from .configs import DiskDataArguments
...
def get_disk_datasets(
    data_config: DiskDataArguments,
    ...
    data_files = data_config.disk_mixer
    ...
    # print("raw_datsets: ", raw_datasets)
    raw_train_datasets = raw_datasets["train"]
    if len(raw_train_datasets) > 0:
        if shuffle:
            raw_datasets["train"] = raw_train_datasets.shuffle(seed=42)
...
def get_disk_sft_datasets(
    ...
    print("tmp_raw:", tmp_raw_datasets)

    for ds_key, dataset in tmp_raw_datasets.items():
        if ds_key not in splits:
            continue
        for split in splits:
            print("split and splits: ", split, splits)
            # Remove redundant columns to avoid schema conflicts on load
            dataset = dataset.remove_columns([col for col in dataset.column_names if col not in columns_to_keep])
            if "train" in split and split == ds_key:
                raw_train_datasets.append(dataset)
            elif "test" in split and split == ds_key:
                raw_val_datasets.append(dataset)
            else:
                print(f"skip {split} comp {ds_key}")
                # raise ValueError(f"Split type {split} not recognized as one of test or train.")
    print("raw_train_datasets: ", raw_train_datasets)
    print("raw_val_datasets: ", raw_val_datasets)
    if len(raw_train_datasets) > 0:
        if shuffle:
